    ...
    public function salvar($disciplina){
        ...
        try {
            if ($disciplina->getIdDisciplina() != "") {
                $statement = $pdo->prepare("UPDATE disciplina SET Professor_idProfessor=:professorDAO, Nome=:nome, Sigla=:sigla, CargaHoraria=:cargaHoraria  WHERE idDisciplina = :id;");
                $statement->bindValue(":id", $disciplina->getIdDisciplina());
            } else {
                $statement = $pdo->prepare("INSERT INTO disciplina (Professor_idProfessor, Nome, Sigla, CargaHoraria) VALUES (:professorDAO, :nome, :sigla, CargaHoraria=:cargaHoraria )");
            }
            $statement->bindValue(":professorDAO",$disciplina->getProfessor());
